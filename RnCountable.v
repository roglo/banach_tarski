...
From Stdlib Require Import Arith ZArith Field.
From RingLike Require Import Utf8 Core IntermVal Misc.
...
destruct bi. {
  ...
  now rewrite (rngl_mul_nat_comm Hos 3).
  apply (rngl_3_neq_0 Hos Hc1 Hto).
  rewrite Hsn3.
...
intros Hc1 *.
unfold partial_sum3.
rewrite (partial_sum3_aux_succ Hc1).
now rewrite rngl_mul_1_r.
Qed.

Theorem partial_sum3_aux_add :
  rngl_characteristic T ≠ 1 →
  ∀ u k₁ k₂ pow i,
...
Theorem partial_sum3_aux_nonneg :
  rngl_characteristic T ≠ 1 →
  ∀ u k pos i,
  (0 ≤ pos)%L
...
Theorem partial_sum3_upper_bound :
  ∀ u n k, (partial_sum3 u k ≤ partial_sum3 u n + (2 * 3 ^ n)⁻¹)%L.
Proof.
destruct_ac.
intros.
...
destruct (le_dec k n) as [Hkn| Hkn]. {
  ...
  subst n.
  rewrite (partial_sum3_aux_add Hc1), Nat.add_0_l, <- rngl_add_assoc.
  apply (rngl_le_add_r Hos Hor).
  apply (rngl_add_nonneg_nonneg Hos Hor); [ | apply Hzi ].
  apply (partial_sum3_aux_nonneg Hc1).
...
rewrite (partial_sum3_aux_add Hc1), Nat.add_0_l.
...
destruct b. {
  ...
  field_simplify. {
    apply (partial_sum3_aux_le_half_pow Hch); [ | easy ].
    apply (rngl_div_nonneg Hop Hiv Hto).
    apply (rngl_div_nonneg Hop Hiv Hto).
    apply (rngl_0_le_1 Hos Hto).
    apply (rngl_pow_pos_pos Hop Hiv Hto).
...
(*
  replace (let (rngl_zero, _, _, _, _, _, _, _) := ro in rngl_zero)
...
induction k; intros; [ simpl; destruct (u i); unfold b2r; simpl | ]. {
  ...
  rewrite rngl_add_comm; apply (rngl_3_neq_0 Hos Hc1 Hto).
  rewrite rngl_add_comm; apply (rngl_3_neq_0 Hos Hc1 Hto).
} {
  rewrite rngl_of_nat_0.
...
rewrite (partial_sum3_aux_succ Hc1), n_partial_sum3_succ.
field_simplify. 2: {
  apply (rngl_pow_neq_0 Hos Hiq).
  apply (rngl_3_neq_0 Hos Hc1 Hto).
}
...
split. {
  ...
  destruct b. {
    ...
    apply (rngl_le_div_r Hop Hiv Hto) in Hr2.
  ...
  apply IHn. {
    unfold partial_sum3.
    apply (rngl_le_div_l Hop Hiv Hto) in Hr1; [ easy | ].
    apply (rngl_0_lt_3 Hos Hc1 Hto).
  }
  progress unfold partial_sum3.
...
induction n. {
  ...
  assert (H : ∀ k, (partial_sum3 u k ≤ 1 / 2)%L). {
    intros k; apply (partial_sum3_aux_le_half_pow Hch); [ | easy ].
    apply (rngl_0_le_1 Hos Hto).
  }
...
rewrite <- rngl_mul_assoc.
rewrite (rngl_mul_inv_diag_l Hiv).
rewrite rngl_mul_1_r.
...
apply (upper_bound_property em Hop Hto Hiv Har Hco _ x _ Hx) in H.
...
Theorem ter_bin_of_frac_part_surj {em : excl_midd} :
  is_complete T rngl_dist →
  ∀ u : nat → bool,
  ∃ r, (0 ≤ r < 1)%L ∧ (∀ n, ter_bin_of_frac_part Hc1 r n = u n).
Proof.
destruct_ac.
intros Hco *.
...
destruct (rngl_ltb_dec (frac_part Hc1 Har (r * 3 ^ n)) (1 / 3)) as [H1| H1]. {
  ...
  apply -> (rngl_lt_div_r Hop Hiv Hto) in H1. {
    rewrite (rngl_div_div Hos Hiv) in H1.
    rewrite <- rngl_pow_succ_l in H1.
    specialize (Hr3 (S n)).
    rewrite (partial_sum3_succ Hc1) in Hr3.
...
apply (rngl_ltb_ge_iff Hto) in H1.
...
rewrite (Int_part_eq_partial_sum3 u) in H1; [ | easy | easy ].
rewrite <- (rngl_mul_sub_distr_r Hop) in H1.
apply -> (rngl_le_div_l Hop Hiv Hto) in H1. {
  rewrite (rngl_div_div Hos Hiv) in H1.
  rewrite <- rngl_pow_succ_l in H1.
  specialize (partial_sum3_upper_bound u (S n)); intros H.
  apply Hr4 in H.
  rewrite (partial_sum3_succ Hc1) in H.
  ...
  rewrite (rngl_inv_mul_distr Hos Hiv) in H.
...
intros Hco.
specialize
  (Cantor_gen ℕ ℕ T (λ x, (0 ≤ x < 1)%L) id (ter_bin_of_frac_part Hc1) id_nat
