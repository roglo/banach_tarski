...
Proof.
destruct_ac.
now apply (rngl_div_1_r Hiq); left.
Qed.
...
induction a as [a| a| ]; intros; cbn. {
  ...
  rewrite IHa.
  rewrite (rngl_mul_add_distr_r _ _ (rngl_of_pos _)), rngl_mul_1_l.
  rewrite rngl_mul_assoc.
  apply rngl_add_comm.
} {
...
destruct a as [a| a| ]; [ | | easy ]. {
  ...
  apply (rngl_eq_mul_0_l Hos Hiq) in Ha1.
  revert Ha1.
  ...
  apply (rngl_of_nat_inj Hos) in H1.
  ...
  destruct (Pos.to_nat a) as [| n]. {
    cbn in Ha1.
    rewrite (rngl_mul_0_r Hos) in Ha1.
    symmetry in Ha1.
    now apply (rngl_1_neq_0 Hc1) in Ha1.
  }
  rewrite rngl_of_nat_succ in Ha1.
  rewrite rngl_mul_add_distr_l in Ha1.
  rewrite rngl_mul_1_r in Ha1.
...
Theorem rngl_of_pos_inj :
  ∀ a b,
  rngl_of_pos a = rngl_of_pos b
  → if Nat.eq_dec (rngl_characteristic T) 0 then a = b
    else Pos.to_nat a ≡ Pos.to_nat b mod rngl_characteristic T.
Proof.
destruct_ac.
intros * Hab.
apply (rngl_of_nat_inj Hos) in Hab.
destruct (Nat.eq_dec (rngl_characteristic T) 0) as [Hch| Hch]; [ | easy ].
...
Proof.
intros Hos * Hij.
apply (rngl_of_nat_inj Hos) in Hij.
destruct (Nat.eq_dec (rngl_characteristic T) 0); [ now subst | easy ].
Qed.
...
induction a as [a| a| ]; intros; [ | | apply Pos.le_1_l ]. {
  destruct b as [b| b| ]. {
    ...
    apply (rngl_mul_le_mono_pos_l Hop Hiq Hto) in Hab. 2: {
    ...
    revert b Hab.
    induction a; intros; [ apply (rngl_of_nat_nonneg Hos Hto) | ].
...
induction a as [a| a| ]; intros. {
  destruct b as [b| b| ]; [ | | easy ]. {
    ...
    apply (rngl_mul_le_mono_pos_l Hop Hiq Hto); [ | now apply IHa ].
    apply (rngl_0_lt_2 Hos Hc1 Hto).
  } {
    apply Pos.compare_le_iff in Hab; cbn in Hab.
...
destruct_ac.
intros *.
now apply (rngl_of_pos_le_or_lt_prop (rngl_lt_le_comp Hto) x).
Qed.
...
...
  apply (rngl_le_div_r Hon Hop Hiv Hiq Hto).
  apply (rngl_0_lt_2 Hos Hc1 Hiq Hto).
...
apply (rngl_lt_div_l Hon Hop Hiv Hiq Hto).
apply (rngl_0_lt_2 Hos Hc1 Hiq Hto).
rewrite rngl_of_pos_add.
rewrite rngl_of_pos_1.
...
(*
intros * Hp.
now apply (gen_rngl_of_pos_xI_interval (rngl_le_lt_comp Hto)).
*)
intros * Hp.
split. {
...
apply (rngl_of_nat_inj_le Hon Hop Hiq Hc1 Hiq Hto) in Hn.
...
destruct c as (c, Hc).
move c before b.
rewrite (rngl_abs_opp Hon Hop Hiq Hto) in Hb.
apply Nat.le_antisymm. {
...
now apply (rngl_div_div Hos Hiv).
Qed.
...
destruct_ac.
intros * Hxy Hyx.
apply (eq_rl_sqrt_0 Hos) in Hyx. {
  apply -> (rngl_sub_move_0_r Hop) in Hyx.
  symmetry in Hyx.
  apply (eq_rngl_squ_rngl_abs Hop Hto Hii _ _ (rngl_mul_comm Hic _ _)) in Hyx.
...
split. {
  ...
  apply (rngl_mul_lt_mono_pos_r Hop Hiq Hto (y⁻¹)). {
    now apply (rngl_inv_pos Hop Hiv Hto).
  }
  rewrite (rngl_mul_sub_distr_r Hop).
  rewrite <- rngl_mul_assoc.
...
destruct (Rcase_abs y) as [Hy| Hy]. {
  do 2 rewrite (rngl_div_opp_r Hop Hiv _ _ Hyz).
  ...
  apply (rngl_nlt_ge_iff Hto) in Hy.
  rewrite (rngl_div_add_distr_r Hiv).
  rewrite (rngl_div_diag Hiq); [ | easy ].
...
rewrite rngl_mul_add_distr_r, rngl_mul_1_l, rngl_add_assoc.
...
destruct (Rcase_abs y) as [Hyn| Hyp]. {
  exfalso; apply (rngl_nle_gt Hor) in Hyn.
  apply Hyn; clear Hyn.
...
destruct (rngl_ltb_dec (frac_part x) (1 / 2)) as [Hx| Hx]. {
  ...
  rewrite <- rngl_mul_2_r.
  apply (rngl_lt_div_r Hop Hiv Hto) in Hx; [ easy | ].
  apply (rngl_0_lt_2 Hos Hc1 Hto).
}
...
destruct n as [| n| n]. {
  ...
  rewrite <- (rngl_pow_div_pow Hos Hiv); [ | | apply Pos2Nat_ge_1 ]. 2: {
    intros H.
    apply (f_equal rngl_opp) in H.
    rewrite (rngl_opp_involutive Hop) in H.
    rewrite (rngl_opp_0 Hop) in H.
    revert H.
    apply rngl_1_neq_0_iff.
    congruence.
  }
  rewrite rngl_pow_1_r, Zabs2Nat.inj_mul.
  simpl (Z.abs_nat 2); unfold Pos.to_nat; simpl (Pos.iter_op _ _ _).
  rewrite (rngl_div_opp_r Hop Hiv).
  rewrite (rngl_div_1_r Hiq); [ | now left ].
  progress f_equal.
  apply (rngl_pow_opp_1_even Hop).
