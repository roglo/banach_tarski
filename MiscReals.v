...
Definition rngl_of_decimal_int (n : Decimal.int) : option T :=
  match n with
  | Decimal.Pos (Decimal.D0 Nil) => Some 0%L
  | Decimal.Pos ui =>
      if rngl_has_1 T then Some (rngl_of_decimal_uint ui)
      else None
  | Decimal.Neg ui =>
      if (rngl_has_1 T && rngl_has_opp T)%bool then
...
Fixpoint nat_of_rngl it a :=
  ...
  | S it' =>
(**)
      match Bool.bool_dec (@rngl_is_ordered T ro) true with
| right _ => None
      | left Hor =>
let Heo := rngl_has_eq_dec_or_is_ordered_r Hor in
      if rngl_eq_dec Heo a a then Some 1
(*
      if rngl_eq_dec Heo a (@rngl_one T ro) then Some 1
*)
      else if rngl_lt_dec Hor a 0 then None
      else
        match nat_of_rngl it' (a - 1)%L with
        | Some n => Some (S n)
        | None => None
        end
  end
...
Compute (nat_of_rngl 1 1%L).
Check rngl_has_eq_dec_or_order.
...
Check (@rngl_has_eq_dec_or_is_ordered_r T ro Hor).
Check Bool.bool_dec.
Check rngl_eq_dec.
rngl_eq_dec
     : rngl_has_eq_dec_or_order T = true → ∀ a b : T, {a = b} + {a ≠ b}
...
...
Fixpoint nat_of_rngl it a :=
  ...
  | 0 => None
  | S it' =>
      if rngl_lt_dec Hor a 0 then None
      else
        match nat_of_rngl it' (a - 1)%L with
        | Some n => Some (S n)
        | None => None
...
Definition rngl_to_decimal_uint (a : T) : option Decimal.uint :=
  match nat_of_rngl a with
  | Some n => Some (Nat.to_uint n)
  | None => None
  end.
*)
...
Definition rngl_of_numeral_int (n : Number.int) : option T :=
  match n with
  | Number.IntDecimal n => rngl_of_decimal_int n
  | Number.IntHexadecimal _ => None
  end.

Definition rngl_to_numeral_uint (a : T) : option Number.uint :=
  match rngl_to_decimal_uint gq with
