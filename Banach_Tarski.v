  ...
  destruct b.
   destruct b1.
    ...
    assert (H : ∀ k, (partial_sum3 u k ≤ 7 / 18)%R).
     ...
     apply Rle_trans with (r2 := (1 / 3 / 3 / 2)%R); [ | lra ].
     apply partial_sum3_aux_le_pow; lra.
   ...
   destruct b1.
    assert (H : ∀ k, (partial_sum3 u k ≤ / 9 + / 2 * / 9)%R).
     intros k.
     unfold partial_sum3.
     destruct k; simpl; [ lra | rewrite Hb ].
     destruct k; simpl; [ lra | rewrite Hb1 ].
    ...
    assert (H : ∀ k, (partial_sum3 u k ≤ / 2 * / 9)%R).
     intros k.
     unfold partial_sum3.
     destruct k; simpl; [ lra | rewrite Hb ].
     destruct k; simpl; [ lra | rewrite Hb1 ].
   ...
   destruct b.
    destruct b1.
     destruct b2.
      ...
      rewrite (Int_part_interv 12); simpl.
       rewrite (Int_part_interv 4); simpl; lra.

       split; [ lra | ].
       ring_simplify.
       enough (r ≤ (25 / 54))%R by lra.
       apply Hr2; intros k.
       unfold partial_sum3.
       destruct k; simpl; [ lra | rewrite Hb ].
       destruct k; simpl; [ lra | rewrite Hb1 ].
    ...
    destruct b1.
     destruct b2.
      assert (H : ∀ k, (partial_sum3 u k ≤ / 9 + / 27 + / 2 * / 27)%R).
       intros k.
       unfold partial_sum3.
       destruct k; simpl; [ lra | rewrite Hb ].
       destruct k; simpl; [ lra | rewrite Hb1 ].
