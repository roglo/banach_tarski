...
Record matrix A (m n : nat) := mkmat
  { def : A; mat : list (list A) }.

Arguments def [A] [m] [n] _.
Arguments mat [A] [m] [n] _.

Definition matel {A m n} (M : matrix A m n) i j :=
  List.nth (pred j) (List.nth (pred i) (mat M) []) (def M).

Definition mat_map {A B m n} (f : A → B) (M : matrix A m n) :=
  mkmat _ m n (f (def M)) (map (map f) (mat M)).

Inductive point := P : ℝ → ℝ → ℝ → point.

Definition mat_vec_mul (M : matrix _ 3 3) '(P x y z) :=
  P (matel M 1 1 * x + matel M 1 2 * y + matel M 1 3 * z)
    (matel M 2 1 * x + matel M 2 2 * y + matel M 2 3 * z)
    (matel M 3 1 * x + matel M 3 2 * y + matel M 3 3 * z).

Definition mkrmat a₁₁ a₁₂ a₁₃ a₂₁ a₂₂ a₂₃ a₃₁ a₃₂ a₃₃ :=
  mkmat ℝ 3 3 0%R [[a₁₁; a₁₂; a₁₃]; [a₂₁; a₂₂; a₂₃]; [a₃₁; a₃₂; a₃₃]].
...
Definition a₁₁ (M : matrix ℝ 3 3) := matel M 1 1.
Definition a₁₂ (M : matrix ℝ 3 3) := matel M 1 2.
Definition a₁₃ (M : matrix ℝ 3 3) := matel M 1 3.
Definition a₂₁ (M : matrix ℝ 3 3) := matel M 2 1.
Definition a₂₂ (M : matrix ℝ 3 3) := matel M 2 2.
Definition a₂₃ (M : matrix ℝ 3 3) := matel M 2 3.
Definition a₃₁ (M : matrix ℝ 3 3) := matel M 3 1.
Definition a₃₂ (M : matrix ℝ 3 3) := matel M 3 2.
Definition a₃₃ (M : matrix ℝ 3 3) := matel M 3 3.
...
Theorem rmat_eq_dec {m n} : ∀ m₁ m₂ : matrix ℝ m n, { m₁ = m₂ } + { m₁ ≠ m₂ }.
Proof.
intros.
pose proof list_eq_dec Req_dec as Rlist_eq_dec.
destruct (Req_dec (def m₁) (def m₂)) as [Hd| Hd].
 destruct (list_eq_dec Rlist_eq_dec (mat m₁) (mat m₂)) as [Hrll| Hrll].
  now left; destruct m₁, m₂; simpl in Hd, Hrll; subst.

  now right; intros H; apply Hrll; subst m₂.

 now right; intros H; apply Hd; subst m₂.
Qed.
